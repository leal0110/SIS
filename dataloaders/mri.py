...
class Segmentation(data.Dataset):
    def __init__(self,
        ...
        _root = 'PATH/TO/MRI'
        dir = ['BIDMC','BMC','HK','I2CVB','RUNMC','UCL','all']
        _root = os.path.join(_root, dir[6])

        _mask_dir = os.path.join(_root, 'label')  # each object each color
        _image_dir = os.path.join(_root, 'img')
        mpath = _mask_dir
        ...
        ml = [f[:-4] for f in filenames]

        for splt in self.split:

            for i in ml:
                nr = i
                _image = os.path.join(_image_dir, nr + '.png')
                _mask = os.path.join(_mask_dir, nr + ".png")

                assert os.path.isfile(_image)
                assert os.path.isfile(_mask)
                self.im_ids.append(nr)
