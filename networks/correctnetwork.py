...
# reverse_channel rgb f g  <-> rgb b f
# raw column IOG error

def generate_distance_map(map_xor, points_center, points_bg, gt, reverse_channel,mask_thres=0.5):
    ...
    correction_new = -np.ones(shape=(gt.shape[0], gt.shape[0], 1), dtype=np.float32)

    sigma = 10

    if map_center == 1:
        points_center = 255 * np.maximum(points_center / 255,
                                         make_gaussian((gt.shape[0], gt.shape[0]), center=[column, raw], sigma=sigma))
        correction_new[make_gaussian((gt.shape[0], gt.shape[0]), center=[column, raw], sigma=3) > mask_thres] = 1
    elif map_bg == 1:
        points_bg = 255 * np.maximum(points_bg / 255,
                                     make_gaussian((gt.shape[0], gt.shape[0]), center=[column, raw], sigma=sigma))
        correction_new[make_gaussian((gt.shape[0], gt.shape[0]), center=[column, raw], sigma=3) > mask_thres] = 0
    else:
        print('error')
    pointsgt_new = np.zeros(shape=(gt.shape[0], gt.shape[0], 2))

    if reverse_channel:
        pointsgt_new[:, :, 0] = points_bg
        pointsgt_new[:, :, 1] = points_center
    else:
    ...
    correction_new = correction_new.transpose((2, 0, 1))
    correction_new = torch.from_numpy(correction_new)

    return pointsgt_new,correction_new


def iou_cal(pre, gts, extreme_points, correction_map, reverse_channel, mask_thres=0.5):
    iu_ave = 0
    distance_map_new = torch.zeros(extreme_points.shape)
    correction_map_new = torch.zeros(extreme_points.shape[0], 1, extreme_points.shape[2], extreme_points.shape[3])
    ...
    for jj in range(int(pre.shape[0])):
        ...
        if reverse_channel:
            points_bg = tens2image(extreme_points[jj, 0:1, :, :])
            points_center = tens2image(extreme_points[jj, 1:2, :, :])
        else:
        ...
        iu_ave = iu_ave + iu
        distance_map_new[jj, :, :, :], correction_map_new[jj, :, :, :] = generate_distance_map(map_xor, points_center,
                                                                                               points_bg, gt, reverse_channel)

    iu_ave = iu_ave / pre.shape[0]
    distance_map_new = distance_map_new.cuda()

    correction_map_new = correction_map_new.cuda()
    correction_map_weight = correction_map_new != -1
    correction_map[correction_map_weight] = correction_map_new[correction_map_weight]

    return iu_ave, distance_map_new ,correction_map
...
class SegmentationNetwork(nn.Module):
    ...
    def forward(self, input, IOG_points, gts, k, distance_map_512, correction_map, reverse_channel=True):
        ...
        # add correction_map
        iou_i, distance_map_new, correction_map_new = iou_cal(fine_out, gts, distance_map_512, correction_map, reverse_channel)

        return coarse_outs[0], coarse_outs[1], coarse_outs[2], coarse_outs[3], \
               fine_out, iou_i, distance_map_new, correction_map_new
    ...
    def _init_reg_params_first_task(self, freeze: List[str] = []):
        ...
        #             spinner='dots').start()
        print('Initializing omega values (first task)')
        # self.backbone
        ...
        print('Successfully initialized omega values (first task)')
    ...
    def _init_reg_params_subseq_tasks(self, freeze: List[str] = []):
        ...
        #             spinner='dots').start()
        print('Initializing omega values (other task)')
        ...
        print('Successfully initialized omega values (other task)')
        self.reg_params = reg_params
    ...
    def _consolidate_reg_params(self):
        ...
        print('Successfully consolidated omega values')
        self.reg_params = reg_params
