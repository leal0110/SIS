...
def generate_distance_map(map_xor, points_center, points_bg, gt, reverse_channel):
...
def iou_cal(pre, gts, extreme_points, reverse_channel, mask_thres=0.5):
    iu_ave = 0
    ...
    for jj in range(int(pre.shape[0])):
        ...
        distance_map_new[jj, :, :, :] = generate_distance_map(map_xor, points_center, points_bg, gt, reverse_channel)
...
class SegmentationNetwork(nn.Module):
    ...
    def forward(self, input, IOG_points, gts, refinement_num_max, reverse_channel=False):
        ...
        for refinement_num in range(0, refinement_num_max):
            ...
            out_512 = F.interpolate(fine_out, size=(512, 512), mode='bilinear', align_corners=True)
            iou_i, distance_map_new = iou_cal(out_512, gts, distance_map_512,reverse_channel)
...
def Network(nInputChannels=5, num_classes=1, backbone='resnet101', output_stride=16,
    ...
    pretrain_coco = True
    if pretrain_coco:
        load_pth_name = 'PATH/TO/PTH'
        print("load coco pretrain")
        pretrain_dict = torch.load(load_pth_name, map_location=lambda storage, loc: storage)
        conv1_weight_new = np.zeros((64, nInputChannels, 7, 7))
        conv1_weight_new[:, :4, :, :] = pretrain_dict['backbone.conv1.weight'].cpu().data
        pretrain_dict['backbone.conv1.weight'] = torch.from_numpy(conv1_weight_new)
        only_backbone = True
        if only_backbone:
        ...
        for k, v in pretrain_dict.items():
            if only_backbone:
                if k in model_dict and k.startswith('backbone.'):
                    state_dict[k] = v
            else:
                if k in model_dict:
                    state_dict[k] = v
